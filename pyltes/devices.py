...
class UE(NetworkDevice):
    ...
    def connectToTheBestBS(self, BS_vector, obstacleVector = None):
        theBestSINR = -1000 #really bad number to start
        foundBS = -1
        for bs in BS_vector:
            if self.isSeenFromBS(bs):
    ...
    def calculateWallLoss(self, BS_vector, obstacleVector):
        ...
        for obstacle in obstacleVector:
            ...
            #if wallLoss != 0:
            #    print("Wall Loss:", wallLoss, obstacle)
        return wallLoss

    def calculateReceivedPower(self, pSend, distance):
        #print(pSend, distance)
        R = distance
        #lambda_val = 0.142758313333 2.1 GHz
        lambda_val = 0.345383 #LoRa
        a = 4.0
        b = 0.0065
        c = 17.1
        d = 10.8
        ...
        #print('Psend:', pSend, 'PL:', PL)
        pRec = pSend - PL
        if(pRec > pSend):
            pRec = pSend
        #print('pRec:', pRec)
        alpha = 20 * math.log10( (4*math.pi*R0p) / lambda_val )
        distance2 = R0*10**((pSend-pRec-alpha-Xf-Xh-s)/(10*gamma))
        if (distance2<R0p):
            distance2 = lambda_val*10**((pSend-pRec-s)/20)/(4*math.pi)
            ...
            if (where=="in" and BS_vector[self.connectedToBS].useSFR):
                ...
                for i in range(1,4):
                    ...
                    if(bs_other.color == i):
                        ...
                        print('outside power IN', bs_other.outsidePower)
                    else:
                        bs_other_power = bs_other.insidePower
                        print('inside power', bs_other.insidePower)
                ...
                if(bs_other.color == myColor):
                    bs_other_power = bs_other.outsidePower
                    print('outside power OUT', bs_other.outsidePower)
                else:
                    bs_other_power = bs_other.insidePower
                    print('inside power', bs_other.insidePower)
